...
#define APP_CFG_NAMESPACE "app_cfg"
...
esp_err_t wifi_prov_get_app_config(app_config_t *cfg)
{
    nvs_handle_t handle;
    esp_err_t err = nvs_open(APP_CFG_NAMESPACE, NVS_READONLY, &handle);
    if (err != ESP_OK) return err;

    size_t len = sizeof(cfg->latitude);
    nvs_get_str(handle, "lat", cfg->latitude, &len);

    len = sizeof(cfg->longitude);
    nvs_get_str(handle, "lon", cfg->longitude, &len);

    uint32_t raw_min, raw_max;
    if (nvs_get_u32(handle, "t_min", &raw_min) == ESP_OK) {
        memcpy(&cfg->temp_min, &raw_min, sizeof(float));
    }
    if (nvs_get_u32(handle, "t_max", &raw_max) == ESP_OK) {
        memcpy(&cfg->temp_max, &raw_max, sizeof(float));
    }
...
esp_err_t wifi_prov_save_app_config(const app_config_t *cfg)
{
    nvs_handle_t handle;
    esp_err_t err = nvs_open(APP_CFG_NAMESPACE, NVS_READWRITE, &handle);
    if (err != ESP_OK) return err;

    nvs_set_str(handle, "lat", cfg->latitude);
    nvs_set_str(handle, "lon", cfg->longitude);

    uint32_t raw_min, raw_max;
    memcpy(&raw_min, &cfg->temp_min, sizeof(float));
    memcpy(&raw_max, &cfg->temp_max, sizeof(float));

    nvs_set_u32(handle, "t_min", raw_min);
    nvs_set_u32(handle, "t_max", raw_max);
